...
resource "local_file" "nginx_config" {
  ...
  content = templatefile("${path.module}/../../terraform/templates/nginx_default.conf.tpl", {
...
resource "local_file" "nginx_stream_config" {
  ...
  content = templatefile("${path.module}/../../terraform/templates/nginx_stream.conf.tpl", {})
...
resource "null_resource" "test_nginx_ssh_connection" {
  ...
  provisioner "remote-exec" {
    inline = [
      "echo 'SSH connection test successful for nginx installation on one-sakura'",
      "whoami",
      "uname -a"
    ]
  }
...
resource "null_resource" "install_nginx" {
  ...
  provisioner "file" {
    source      = local_file.nginx_install_script.filename
    destination = "/tmp/install_nginx.sh"
  }

  # Upload nginx configuration
  provisioner "file" {
    source      = local_file.nginx_config.filename
    destination = "/tmp/nginx.conf"
  }

  # Upload nginx stream configuration
  provisioner "file" {
    source      = local_file.nginx_stream_config.filename
...
EOT
  ...
  triggers = {
    script_hash = local_file.nginx_install_script.content_md5
    config_hash = local_file.nginx_config.content_md5
    stream_hash = local_file.nginx_stream_config.content_md5
  }
}
