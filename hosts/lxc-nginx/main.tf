...
resource "local_file" "lxc_nginx_install_script" {
  ...
  content = templatefile("${path.module}/../../terraform/templates/nginx_install.sh.tpl", {})
...
resource "local_file" "lxc_nginx_config" {
  ...
  content = templatefile("${path.module}/../../terraform/templates/lxc_nginx.conf.tpl", {
...
resource "local_file" "lxc_nginx_stream_config" {
  ...
  content = templatefile("${path.module}/../../terraform/templates/lxc_stream.conf.tpl", {})
...
resource "null_resource" "test_lxc_nginx_ssh_connection" {
  ...
  provisioner "remote-exec" {
    inline = [
      "echo 'SSH connection test successful for LXC nginx installation on lxc-nginx'",
      "whoami",
      "uname -a"
    ]
  }
...
resource "null_resource" "install_lxc_nginx" {
  ...
  provisioner "file" {
    source      = local_file.lxc_nginx_install_script.filename
  ...
  provisioner "file" {
    source      = local_file.lxc_nginx_config.filename
    destination = "/tmp/nginx.conf"
  }

  # Upload nginx stream configuration
  provisioner "file" {
    source      = local_file.lxc_nginx_stream_config.filename
  ...
  triggers = {
    script_hash = local_file.lxc_nginx_install_script.content_md5
    config_hash = local_file.lxc_nginx_config.content_md5
    stream_hash = local_file.lxc_nginx_stream_config.content_md5
  }
}
