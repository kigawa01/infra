...
resource "null_resource" "install_lxc_nginx" {
  ...
  provisioner "file" {
    ...
    destination = local.nginx.install.script.path
  }
  ...
  provisioner "remote-exec" {
    inline = [
      "chmod +x ${local.nginx.install.script.path}",
      # Set non-interactive installation options
      "export DEBIAN_FRONTEND=noninteractive",
      "export UCF_FORCE_CONFFNEW=YES",
      "export NEEDRESTART_MODE=a",
      local.nginx.install.script.path,
    ]
  }
  # Upload nginx configuration
  provisioner "file" {
  ...
  provisioner "file" {
    ...
    destination = "/tmp/lxc_proxy.stream.conf"
  }
  ...
  provisioner "remote-exec" {
    inline = [
      ...
      "cp /tmp/lxc_proxy.stream.conf /etc/nginx/stream.conf.d/lxc_proxy.stream.conf",
      "nginx -t",
